...
class OpenVPNTunnelConnection {
    ...
    private weak var bridge: OpenVPNProxyBridge?
    ...
    private func resolveAndConnect() async {
        ...
        guard let destinationIP = await bridge.resolveHostname(targetHost) else {
            logger.log("OpenVPNTunnel: DNS resolve failed for \(targetHost)", category: .vpn, level: .error)
        ...
        session.onError = { [weak self] error in
            guard let self, !self.isCancelled else { return }
            self.logger.log("OpenVPNTunnel: TCP error for \(self.targetHost):\(self.targetPort) - \(error)", category: .vpn, level: .error)
            self.finish(error: true)
        }

        bridge.initiateConnection(session)
        logger.log("OpenVPNTunnel: connecting to \(targetHost):\(targetPort) via WG tunnel", category: .vpn, level: .debug)
    }
    ...
    private func startTimeout() {
        let work = DispatchWorkItem { [weak self] in
            Task { @MainActor [weak self] in
                guard let self, !self.isCancelled else { return }
                self.logger.log("OpenVPNTunnel: timeout for \(self.targetHost):\(self.targetPort)", category: .vpn, level: .warning)
                self.finish(error: true)
            }
        }
        timeoutWork = work
